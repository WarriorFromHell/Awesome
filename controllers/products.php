...
class products extends Controller
{
    ...
    function buy()
    {
        ...
        // Valida form data
        try {
            if (!v::string()->length(5, 50)->validate(($customer['customer_name']))) throw new Exception("Customer name incorrect!");
            if (!v::phone()->length(5, 20)->validate(($customer['customer_number']))) throw new Exception("Customer phone incorrect!");
            if (!v::string()->length(6, 50)->validate(($customer['customer_address']))) throw new Exception("Customer address incorrect!");
            if (!v::email()->validate($customer['customer_email'])) throw new Exception("Customer email incorrect!");
        } catch (Exception $e) {
